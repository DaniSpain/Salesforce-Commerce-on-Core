...
import { resolve } from 'c/b2bUpchargeCmsResourceResolver';
import { getLabelForOriginalPrice, displayOriginalPrice } from 'c/b2bUpchargeCartUtils';
...
export default class Items extends NavigationMixin(LightningElement) {
    ...
    set cartItems(items) {
        console.log("upchargeCartItems::set cartItems");
        console.log(items);
        ...
        Promise.all(generatedUrls).then(() => {
            ...
            if (this._items && this._items.length>0) {
                //create a specific object to keep configurations
                console.log("setting configuration");
                for (var i=0; i<this._items.length; i++) {
                    var item = { ...this._items[i] };
                    if (item.cartItem.isConfiguration) {
                        if (item.cartItem.isBundleParent) {
                            item["fullConfigPrice"] = parseFloat(item.cartItem.totalPrice);
                            item["options"] = [];
                            //now get the options that has the parent as the current id
                            for (var j=0; j<this._items.length; j++) {
                                var option = { ...this._items[j] };
                                console.log("Option");
                                console.log(option);
                                if (option.cartItem.parentItem == item.cartItem.cartItemId) {
                                    item.options.push(option);
                                    //sum the config price
                                    item["fullConfigPrice"] += parseFloat(option.cartItem.totalPrice);
                                }
                            }
                            //calculate the total price of the configuration

                            this.configurationItems.push(item);
                        }
                    }
                }
                console.log(this.configurationItems);
            }
        });
    }

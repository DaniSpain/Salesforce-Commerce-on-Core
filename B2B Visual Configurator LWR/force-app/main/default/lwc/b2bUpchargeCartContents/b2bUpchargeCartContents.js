...
import getCartItems from '@salesforce/apex/B2BUpchargeCartController.getCartItems';
import updateCartItem from '@salesforce/apex/B2BUpchargeCartController.updateCartItem';
import deleteCartItem from '@salesforce/apex/B2BUpchargeCartController.deleteCartItem';
import deleteCart from '@salesforce/apex/B2BUpchargeCartController.deleteCart';
import createCart from '@salesforce/apex/B2BUpchargeCartController.createCart';
import getUpchargeConfiguration from '@salesforce/apex/B2BUpchargeCartController.getUpchargeConfiguration';

import { fireEvent } from 'c/b2bUpchargePubsub';
import { isCartClosed } from 'c/b2bUpchargeCartUtils';

//TODO: show promotions in cart
//TODO: show calculated sku
...
export default class CartContents extends NavigationMixin(LightningElement) {
    ...
    updateCartItems() {
        ...
        getCartItems({
            ...
            .then((result) => {
                console.log("bundleCartContent::getCartItems::success");
                console.log(result.cartItems);

                //DSPAGNUOLO: get cart items configuration
                getUpchargeConfiguration({
                    communityId: communityId,
                    effectiveAccountId: this.resolvedEffectiveAccountId,
                    activeCartOrId: this.recordId,
                }).then((configResult) => {
                    console.log("getUpchargeConfiguration::success");
                    ...
                    if (configResult && configResult.length > 0) {
                        for (var i=0; i<this.cartItems.length; i++) {
                            ...
                            for (var j=0; j<configResult.length; j++) {
                                var confElem = configResult[j];
                                if (confElem.cartItemId == cartItem.cartItem.cartItemId) {
                                    cartItem.cartItem["isBundleParent"] = confElem.isBundleParent;
                                    cartItem.cartItem["parentItem"] = confElem.parentItem;
                                    cartItem.cartItem["isConfiguration"] = true;
                    ...
                    console.log("getUpchargeConfiguration::enriched cart items");
                    console.log(this.cartItems);
                }).catch((error) => {
                    console.error("getUpchargeConfiguration::error");
                    console.error(error);
                });
